...
# ─────────────────────────────────────────────────────────
# GEX44 LLM tunnel management
# ─────────────────────────────────────────────────────────

tunnel-up() {
  systemctl --user start llm-tunnel.service
  sleep 1
  if ss -tlnp 2>/dev/null | grep -q 127.0.0.1:8080; then
    echo "✓ Tunnel up — http://localhost:8080"
  else
...
tunnel-down() {
  systemctl --user stop llm-tunnel.service
  echo "✓ Tunnel stopped"
}

tunnel-restart() {
  systemctl --user restart llm-tunnel.service
  sleep 2
  tunnel-status
}
...
tunnel-status() {
  local active
  active=$(systemctl --user is-active llm-tunnel.service)

  if [[ "$active" == "active" ]]; then
    if ss -tlnp 2>/dev/null | grep -q 127.0.0.1:8080; then
      echo "✓ Service: active   Port: listening   URL: http://localhost:8080"
    else
      echo "⚠ Service: active   Port: NOT LISTENING (reconnecting?)"
    fi
  else
    echo "✗ Service: $active"
  fi
...
tunnel-logs() {
  journalctl --user -u llm-tunnel.service -f --since "10 minutes ago"
}

tunnel-test() {
  if ! command -v jq >/dev/null; then
    echo "jq not installed. sudo pacman -S jq"
    return 1
  fi
  ...
  if [[ -z "$response" || "$response" == "ERROR" ]]; then
    echo "✗ LLM unreachable. Run: tunnel-status"
    return 1
  fi

  echo "✓ LLM responded: $response"
}

tunnel() {
  if systemctl --user is-active --quiet llm-tunnel.service; then
    tunnel-down
  else
    tunnel-up
  fi
...
ask() {
  # Capture all arguments passed to the function as a single string
  local prompt="$*"

  # Validate that an argument was actually provided
  if [[ -z "$prompt" ]]; then
    echo "Error: No prompt provided."
    echo "Usage: ask <your message>"
    return 1
  fi

  # Safely construct the JSON payload using jq to prevent syntax errors from unescaped quotes
  local json_payload
